...
def load_api_data_range(start_date, end_date):
    ...
    Проходим в цикле по каждому дню, собираем ID чатов, потом анализируем.
    """
    
    # 1. Загрузка операторов (один раз)
    try:
    ...
    for i, d_str in enumerate(date_list):
        ...
        progress_bar.progress((i + 1) / (len(date_list) * 2)) # Половина прогресса - это сбор

    # Убираем дубликаты (если один чат попал в разные дни)
    # Хотя request_stats по дате обычно уникален по активности, но перестрахуемся
    ...
    # Глобальные границы диапазона для фильтрации сообщений
...
if st.sidebar.button("Запустить анализ (API)"):
    ...
    # Чистим кэш только если даты изменились, но тут для надежности
    st.cache_data.clear()
...
# ЗАГРУЗКА
df_api, speeds_map = load_api_data_range(sel_start, sel_end)

# Фильтр GSheet по диапазону
...
with tabs[2]:
    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            # --- ИСПРАВЛЕНИЕ ОШИБКИ (Безопасный рейтинг) ---
            ...
            # Форматируем строку рейтинга с проверкой
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt}))**
            ...
            for i, row in op_stats.iterrows():
                ...
                with st.expander(f"{i+1}. {op_name}: {cnt} чатов (Ср: {s_time} | Оценка: {s_rate})"):
                    st.write(f"ID: {op_id}")
                    st.write(f"Всего ответов: {len(s_speeds)}")
