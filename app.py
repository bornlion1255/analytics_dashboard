...
# TAB 4: Анализ категорий (Все обращения + ТОП-15)
with tabs[3]:
    st.subheader("📊 Анализ типов обращений")
    
    if not df_gsheet.empty:
        # --- БЛОК ТОП-15 (ВИЗУАЛИЗАЦИЯ) ---
        st.write("#### 🏆 ТОП-15 самых частых категорий")
        
        # Считаем частоту
        top_categories = df_gsheet['Тип обращения'].value_counts().nlargest(15).reset_index()
        top_categories.columns = ['Категория', 'Кол-во']
        
        col_chart, col_list = st.columns([2, 1])
        
        with col_chart:
            # Рисуем красивый горизонтальный график
            fig_top, ax_top = plt.subplots(figsize=(8, 5))
            sns.barplot(data=top_categories, x='Кол-во', y='Категория', palette='viridis', ax=ax_top)
            ax_top.set_xlabel("Количество чатов")
            ax_top.set_ylabel("")
            st.pyplot(fig_top)
            
        with col_list:
            # Компактный список для быстрого чтения
            st.write("**Краткий список:**")
            for i, row in top_categories.iterrows():
                st.write(f"{i+1}. {row['Категория']} — **{row['Кол-во']}**")

        st.divider()

        # --- ОСНОВНАЯ ТАБЛИЦА (ДЕТАЛИЗАЦИЯ) ---
        st.write("#### 📝 Детальная статистика по всем типам")
        
        # Группируем ВСЕ данные из таблицы по теме и статусу
        ...
        # Расчет эффективности бота
        stats['Бот(✓)'] = (stats['Закрыл'] / stats['Всего'] * 100).map('{:.1f}%'.format)
        stats['Бот(→)'] = (stats['Перевод'] / stats['Всего'] * 100).map('{:.1f}%'.format)

        # Детализация причин перевода
        reasons = ['Требует сценарий', 'Не знает ответ', 'Лимит сообщений']
        tr_df = df_gsheet[df_gsheet['Статус'] == 'Перевод']
        
        if not tr_df.empty:
            r_counts = tr_df.groupby('Тип обращения')['Причина перевода'].value_counts().unstack(fill_value=0)
            for r in reasons:
                if r not in r_counts.columns: r_counts[r] = 0
            stats = stats.join(r_counts, how='left').fillna(0)
        else:
            for r in reasons: stats[r] = 0

        def fmt_details(row):
            transferred = row.get('Перевод', 0)
            if transferred == 0: return "—"
            res = [f"• {r}: {(row.get(r,0)/transferred*100):.0f}%" for r in reasons if row.get(r,0) > 0]
            return "\n".join(res) if res else "• Другое/Автоматика"

        stats['Детализация перевода'] = stats.apply(fmt_details, axis=1)
        
        final = stats[['Всего', 'Бот(✓)', 'Бот(→)', 'Детализация перевода']].sort_values('Всего', ascending=False).reset_index()
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Всего": st.column_config.NumberColumn("Всего чатов"),
                "Бот(✓)": st.column_config.TextColumn("Закрыто ботом", help="Успешно завершил бот"),
                "Бот(→)": st.column_config.TextColumn("Ушло оператору", help="Процент переводов"),
                "Детализация перевода": st.column_config.TextColumn("Причины перевода", width="medium")
            }
        )
    else:
