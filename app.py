...
# ==========================================
# TAB 3: DEPT ANALYSIS (С ТИМЛИДАМИ В НАГРУЗКЕ)
# ==========================================
with tabs[2]:
    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            # Логика Тимлидов (Оставляем только для визуальной подсветки в таблице)
            ...
            def calc_metrics_for_report(df, sm, fsm, dept_name):
                ...
                if dept_name == "Бот AI":
                    ...
                    # ИСПРАВЛЕНИЕ: Считаем всех операторов, включая тимлидов (убрали исключение is_tl)
                    daily_o = df.groupby('Дата')['operator_id'].nunique() if not df.empty else pd.Series(dtype=float)
...
Средняя скорость: {format_seconds(curr_m['speed'])}{fmt_trend(curr_m['speed'], prev_m['speed'] if prev_m else 0, is_time=True)}"""
            ...
            if selected_dept == "Бот AI":
                ...
                # ИСПРАВЛЕНИЕ: Считаем всех операторов, включая тимлидов
                daily_ops = dept_data.groupby('Дата')['operator_id'].nunique()
            else:
