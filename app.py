                ...
                if client_waiting_since is None: client_waiting_since = dt_local
                 ...
                 if target_start <= dt_local <= target_end:
                     ...
                     if client_waiting_since:
                         ...
                         if diff > 0:
                             if op_id not in stats['operator_speeds']: stats['operator_speeds'][op_id] = []
...
def load_api_data(date_str):
    ...
    while offset < 5000:
        try:
            ...
            for row in data:
                ...
                active_requests.append({'req_id': row['request_id'], 'rating': rating})
            if len(data) < limit: break
            offset += limit
        except: break
        
    final_rows = []
    all_speeds = {} # op_id -> [speeds]
    
    progress_bar = st.progress(0)
    status_text = st.empty()
    total = len(active_requests); completed = 0
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            completed += 1
            if total > 0: progress_bar.progress(min(completed / total, 1.0))
            status_text.text(f"Обработано API: {completed}/{total}")
            
    progress_bar.empty(); status_text.empty()
    
    df = pd.DataFrame(final_rows)
    return df, all_speeds
...
if not df_gsheet_all.empty:
    default_date = df_gsheet_all['Дата'].max().date()
else:
    default_date = datetime.now().date()

selected_date = st.sidebar.date_input("Дата анализа", value=default_date)
...
if 'run_analysis' not in st.session_state:
    st.info("👈 Выберите дату и нажмите 'Запустить анализ'"); st.stop()

date_str = selected_date.strftime("%Y-%m-%d")

# ЗАГРУЗКА
df_api, speeds_map = load_api_data(date_str)
...
with tabs[0]:
    ...
    with col_pies[1]:
        ...
        if participated_count > 0:
            ...
            st.write("Бот не участвовал в диалогах за этот день.")
...
with tabs[1]:
    if not df_api.empty:
        ...
        with c_heat:
            st.write("**Тепловая карта: Операторы**")
            # Группировка для API (часы операторов)
            # В process_single_dialog мы не вытаскивали час каждого ответа для DF, но у нас есть df_api с колонкой 'Час' если добавить
            # В текущей реализации load_api_data мы не сохраняли час. Давайте это опустим или добавим.
            # UPD: Лучше построить по GSheet темам, как вы просили "все обращения"
            pass

    st.subheader("Тематика обращений по времени (ВСЕ обращения)")
    # НЕ убираем прочерки, чтобы видеть все
...
with tabs[2]:
    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            # 1. ОБЩАЯ СВОДКА ПО ОТДЕЛУ
            # Скорость по отделу
            dept_speeds = []
            dept_ratings = []
            
            # Собираем всех операторов отдела
            ...
            # Рейтинг по отделу (среднее из строк df_api)
            # Примечание: в df_api одна строка = одно участие. Рейтинг дублируется если оператор писал много раз? 
            # Нет, мы сделали 1 строку на оператора в чате.
            # Но если в чате 2 оператора, рейтинг дублируется. 
            # Корректно: брать уникальные req_id и их рейтинги.
            unique_ratings = dept_data.drop_duplicates('req_id')['rating'].dropna().astype(float)
            
            d_chats = dept_data['req_id'].nunique()
            d_med = np.median(dept_speeds) if dept_speeds else None
            d_rate = unique_ratings.mean() if not unique_ratings.empty else None
            d_rate_cnt = len(unique_ratings)
            
            st.markdown(f"""
            ### 📂 {selected_dept}: {d_chats} чатов
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate:.2f} ({d_rate_cnt}))**
            """)
            
            st.divider()
            
            # 2. ДЕТАЛИЗАЦИЯ ПО СОТРУДНИКАМ
            # Группируем df_api по оператору
            op_stats = dept_data.groupby(['operator_id', 'Оператор']).agg(
                chats=('req_id', 'nunique'),
                rating_mean=('rating', 'mean') 
            ).reset_index().sort_values('chats', ascending=False)
            
            for i, row in op_stats.iterrows():
                ...
                s_time = format_seconds(s_med)
                
                # Рейтинг сотрудника (средний по его чатам)
                # Нужно брать не mean из groupby (там может быть nan), а аккуратно
                op_ratings = dept_data[dept_data['operator_id'] == op_id]['rating'].dropna().astype(float)
                s_rate = f"{op_ratings.mean():.1f}" if not op_ratings.empty else "-"
                
                # Выводим красиво
                with st.expander(f"{i+1}. {op_name}: {cnt} чатов (Ср: {s_time} | Оценка: {s_rate})"):
                    st.write(f"ID: {op_id}")
                    st.write(f"Всего ответов (сообщений): {len(s_speeds)}")
