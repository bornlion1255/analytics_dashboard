...
# 1. Какие ключи вообще есть?
print(f"Доступные ключи в st.secrets: {list(st.secrets.keys())}")

# 2. Проверка ключа PASSWORD
if "PASSWORD" in st.secrets:
    real_pass = str(st.secrets["PASSWORD"])
    print(f"✅ Ключ PASSWORD найден.")
    print(f"Тип данных: {type(st.secrets['PASSWORD'])}")
    print(f"Длина пароля: {len(real_pass)} символов")
    print(f"Первый символ: '{real_pass[0]}'")
    print(f"Последний символ: '{real_pass[-1]}'")
    
    # Проверка на пробелы
    if len(real_pass) != len(real_pass.strip()):
        print("⚠️ ВНИМАНИЕ: В пароле есть скрытые пробелы в начале или конце!")
    else:
        print("Пробелов в начале и конце нет.")
else:
    print("❌ Ключ PASSWORD НЕ НАЙДЕН в секретах!")
...
# --- ЗАГРУЗКА СЕКРЕТОВ ---
# Если секретов нет, выводим ошибку, чтобы не гадать, почему не работает
try:
    API_TOKEN = st.secrets["API_TOKEN"]
    SHEET_ID = st.secrets["SHEET_ID"]
    GID = st.secrets["GID"]
except Exception as e:
    st.error(f"❌ Ошибка конфигурации: Не найдены секреты! Проверьте настройки Streamlit Cloud. Детали: {e}")
    st.stop()
...
def check_password():
    ...
    if not st.session_state["password_correct"]:
        ...
        # --- ОТЛАДКА: ПРОВЕРЯЕМ, ЧИТАЮТСЯ ЛИ СЕКРЕТЫ ---
        # (Удалите этот блок после того, как всё заработает!)
        try:
            stored_pass = st.secrets["PASSWORD"]
            # st.info(f"DEBUG: Пароль загружен из Secrets. Первые 2 символа: {stored_pass[:2]}***") 
        except KeyError:
            st.error("ОШИБКА: В Secrets не найден ключ 'PASSWORD'. Проверьте настройки на streamlit.io")
            st.stop()
        # ------------------------------------------------
        ...
        with st.form("credentials"):
            ...
            if submit:
                # Сравниваем введенное с тем, что в секретах
                # .strip() убирает случайные пробелы, которые могли попасть при копировании
                if password.strip() == stored_pass.strip():
...
def process_single_dialog(item, target_start, target_end):
    ...
    try:
        ...
        stats = {
            'req_id': req_id,
            'participations': set(), # op_id
            'operator_speeds': {},   # op_id -> list of speeds
            'op_hours': {},          # op_id -> set of hours (ДЛЯ ТЕПЛОВОЙ КАРТЫ)
            ...
            if msg_type == 'from_client' or msg_type == 'in':
                if client_waiting_since is None: client_waiting_since = dt_local
                 ...
                 if target_start <= dt_local <= target_end:
                     ...
                     # Фиксируем час активности этого оператора
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res and res['participations']:
                ...
                # Строки для DF (разворачиваем по часам для тепловой карты)
                for op_id in res['participations']:
                    ...
                    # Получаем часы активности этого оператора
...
with tabs[1]:
    ...
    if not df_api.empty:
        ...
        with c_heat:
            ...
            # Строим карту по данным API (df_api теперь содержит строки с часами)
