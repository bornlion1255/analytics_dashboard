        ...
        for future in as_completed(futures):
            ...
            if res and res['participations']:
                ...
                for op_id in res['participations']:
                    ...
                    if not hours:
                        # Если вдруг часов нет (странно, но бывает), ставим хотя бы -1 или 0, или пропускаем
                        # Но лучше добавить одну запись без часа для учета KPI
                        final_rows.append({
                            'req_id': res['req_id'],
                            'operator_id': op_id,
                            'Оператор': op_name,
...
with tabs[1]:
    ...
    if not df_api.empty:
        ...
        with c_heat:
            ...
            # Фильтруем валидные часы (0-23)
            hm_df = df_api[df_api['Час'].between(0, 23)]
            
            if not hm_df.empty:
                # nunique req_id - количество уникальных диалогов в этот час
                ...
                st.warning("Нет данных по часам в API (возможно, нет сообщений от операторов).")
