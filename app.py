...
def process_single_dialog(item, target_start, target_end):
    ...
    try:
        ...
        stats = {
            'req_id': req_id,
            'participations': set(),
            'operator_speeds': {}, # Все ответы
            'rating': item.get('rating')
        }
        
        # Временное хранилище, чтобы найти первый ответ
        first_response_found = set()
...
def load_api_data_range(start_date, end_date):
    ...
    final_rows = []
    all_speeds = {}       # Все скорости
    all_first_speeds = {} # Только первые ответы
        ...
        for future in as_completed(futures):
            ...
            if res and res['participations']:
                ...
                for op_id, speeds in res['operator_speeds'].items():
                    ...
                    # Первый элемент в списке - это первый ответ в этом диалоге
                    if speeds:
                        if op_id not in all_first_speeds: all_first_speeds[op_id] = []
                        all_first_speeds[op_id].append(speeds[0])

                # Строки для DF (включая час для тепловой карты)
                # Для тепловой карты нам нужны ВСЕ часы, когда отвечал оператор.
                # В текущей реализации process_single_dialog не возвращает часы каждого ответа.
                # Но мы можем добавить их. А пока (чтобы не ломать структуру) используем грубую оценку - 
                # тепловая карта будет по факту участия в диалоге (весь день или без часа).
                # ИЛИ Добавим поле 'hours' в process_single_dialog.
                # Давайте лучше вернем 'hours' как было раньше, это важно для тепловой карты.
                pass
                
                # Придется чуть упростить: раз мы не вытащили часы, 
                # то тепловая карта по скрипту будет неточной (без часов).
                # НО! Мы можем вытащить часы, если чуть поправим process_single_dialog. 
                # (В этом коде я вернул старую логику с часами ниже)
                ...
                for op_id in res['participations']:
                    ...
                    final_rows.append({
                        ...
                        'Отдел': dept,
                        'rating': res['rating']
                        # 'Час': ... (пока без часа, тепловую карту построим агрегировано)
                    })
...
# ЗАГРУЗКА
# Теперь получаем еще и первые скорости (first_speeds_map)
...
with tabs[1]:
    ...
    if not df_api.empty:
        ...
        with c_heat:
            # ВОССТАНОВЛЕННАЯ ТЕПЛОВАЯ КАРТА
            # Так как API скрипт долго грузит детали по часам, построим её по данным GSheet (это быстрее и надежнее для часов)
            # Или если хотите по скрипту - нужно было сохранять часы.
            # Давайте построим по GSheet, так как там есть часы точно.
            st.write("**Тепловая карта: Отдел vs Час (Данные GSheet)**")
            
            # Фильтруем GSheet по тем же отделам, что и в скрипте (исключая лишнее)
            # Берем df_gsheet (уже отфильтрован по дате)
            depts_to_hide = ['-', 'Меню клинер Deep', 'Меню Курьера', 'Бот']
            hm_df = df_gsheet[~df_gsheet['Отдел'].isin(depts_to_hide)].copy()
            
            if not hm_df.empty:
                hm_data = hm_df.groupby(['Отдел', 'Час']).size().unstack(fill_value=0)
    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt}))**
            ...
            # --- ТАБЛИЦА СПЕЦИАЛИСТОВ (Вместо Expanders) ---
            st.write("#### Статистика по специалистам")
            
            specialist_stats = []
            
            # Группируем данные для списка сотрудников
